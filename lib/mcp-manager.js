...
function buildMcpClientConfig(serverName, cfg) {
  ...
  if (cfg.transport === 'stdio') {
    // Split "npx -y package /tmp" → command + args for the MCP client
    const parts = String(cfg.command ?? '').split(/\s+/).filter(Boolean)
    base.command = parts[0] ?? ''
    base.args = cfg.args && cfg.args.length ? cfg.args : parts.slice(1)
    // Only inherit PATH + HOME; full process.env may contain non-string values
    // that fail the dsh-mcp-client Config schema (z.dict(z.string())).
    base.env = {}
    if (process.env.PATH) base.env.PATH = process.env.PATH
    if (process.env.HOME) base.env.HOME = process.env.HOME
    if (cfg.env) Object.assign(base.env, cfg.env)
    // Default cwd to /tmp so npx doesn't scan DSH's directory
    base.cwd = cfg.cwd || '/tmp'
  } else {
    base.url = cfg.url
