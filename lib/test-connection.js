...
export async function testConnection(config) {
  ...
  switch (config.transport) {
    case 'stdio': {
      ...
      // Split "npx -y @anthropic-ai/mcp-server-filesystem /tmp" → command + args
      const parts = config.command.split(/\s+/).filter(Boolean)
      const cmd = parts[0]
      const parsedArgs = parts.slice(1)
      const cwd = config.cwd || tmpdir()
      // Inherit parent env but reset PWD / npm_config_cache so npx runs cleanly
      const env = { ...process.env, PWD: cwd, npm_config_cache: undefined }
      if (config.env) Object.assign(env, config.env)
      transport = new StdioClientTransport({
        command: cmd,
        args: config.args && config.args.length ? config.args : parsedArgs,
        env: env,
        stderr: 'pipe',
        cwd: cwd,
      })
