window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // ── Settings backend (read/write via api.settings) ──
    var SETTINGS_NS = "mcp-manager";

    // Cache of the last-known settings revision + connection list
    var settingsCache = null; // { revision: number, connections: [] }

    function loadFromSettings(api) {
      if (!api || !api.settings) return [];
      return api.settings.describe({}).then(function(res) {
        if (!res.result || !res.result.ok) return [];
        var namespaces = res.result.value.namespaces || [];
        var found = namespaces.filter(function(ns) { return ns.ns === SETTINGS_NS; });
        if (found.length === 0) return [];
        var ns = found[0];
        settingsCache = { revision: ns.revision, connections: Array.isArray(ns.value) ? ns.value : [] };
        return settingsCache.connections;
      }).catch(function() { return []; });
    }

    function persistConnections(api, connections, oldRevision) {
      if (!api || !api.settings) return;
      api.settings.mutate({
        ns: SETTINGS_NS,
        ops: [{ op: "set", path: [], value: connections }],
        expectedRevision: oldRevision,
      }).then(function(res) {
        if (res.result && res.result.ok && res.result.value) {
          settingsCache = { revision: res.result.value.revision, connections: connections };
        }
      }).catch(function() {});
    }

    // Fallback: sessionStorage for non-persistent state (tools results)
    ...
    function McpManagerCard(props) {
      ...
      var _loaded = React.useState(false);

      // On mount: load connections from settings API
      React.useEffect(function() {
        if (api && api.settings) {
          loadFromSettings(api).then(function(remote) {
            if (remote && remote.length > 0) {
              setConns(remote);
              // Also sync to sessionStorage for offline read
              saveStored(remote);
            }
          });
        }
      ...
      function persistAll(api, list) {
        if (!api || !api.settings) return;
        var rev = settingsCache ? settingsCache.revision : undefined;
        ...
        persistConnections(api, clean, rev);
      }
    ...
    exports.apply = function apply(ctx) {
      ...
      // Get api from connection service
      var api = null;
      try { var connHandle = ctx.get("connection"); if (connHandle && connHandle.api) api = connHandle.api; } catch(_) {}

      ctx.slots.inject("settings.plugin.item", function() {
        return ctx.slots.register({ name: "settings.plugin.item", id: "mcp-manager", order: 100, locale: NS, inject: function() { return { api: api }; }, }, function(props) {
          return jsx.jsx(McpManagerCard, { api: api });
        });
      });
    };
